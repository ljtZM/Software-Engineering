...
db = SQLAlchemy(app)
app.config['JWT_SECRET_KEY'] = 'a9fdg#12!@#A3fFj9z_Secret987'  # 设置密钥
jwt = JWTManager(app)
...
#     id = db.Column(db.Integer, primary_key=True)
#     username = db.Column(db.String(64), nullable=False)   # 操作用户
#     operation = db.Column(db.String(128), nullable=False)  # 操作类型
#     details = db.Column(db.Text, nullable=True)            # 操作详情
#     success = db.Column(db.Boolean, default=True)          # 操作结果
...
#     db.session.commit()
#     # 记录日志（假设管理员ID为1）
#     log = OperationLog(id=1, action=f"新增用户: {username}")
#     db.session.add(log)
#     db.session.commit()
...
#     query = db.session.query(OperationLog, User).select_from(OperationLog).join(User, OperationLog.username == User.id)

#     if keyword:
#         query = query.filter(OperationLog.action.like(f'%{keyword}%'))
...
#     log = OperationLog(id=1, action=f"删除用户: {username}")
#     db.session.add(log)
#     db.session.commit()
...
#     log = OperationLog(id=1, action=f"修改用户 {old_username} 的信息")
#     db.session.add(log)
#     db.session.commit()
#     return jsonify(user.to_dict())
